...
{
    echo "${MTPROXYMAX_PORT}"               # MTProxy/MTProxyMax port
    echo ""                                 # MTProxyMax listen IP, takes the default
    echo "4"                                # customise FakeTLS domain
    echo "${MTPROXYMAX_FAKETLS_DOMAIN}"     # FakeTLS domain
    echo "y"                                # Enable traffic masking
    echo "n"                                # Ad-tag
    echo "${MTPROXYMAX_CPU_LIMIT}"          # Resource limits: CPU
    echo "${MTPROXYMAX_MEMORY_LIMIT}"       # Resource limits: memory
